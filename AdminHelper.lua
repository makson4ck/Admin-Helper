...
if bSettings.forms.isFormFindError[0] then
    if text:find("^%[Ошибка%] {FFFFFF}(.+)") then
        local formaerror = text:match('^%[Ошибка%] {FFFFFF}(.+)')
        if formaerror == "Не флуди!" then
            --nothing
        else
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, 'Используйте: '..formaerror)
    end
end
    if text:find("^%[Ошибка%] {FFFFFF}Не флуди!") then
        bSettings.forms.isError[0] = false
    end
    if text:find("Используй: (.+)") then
        local formaerror = text:match('Используй: (.+)')
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, 'Используйте: '..formaerror)
    end
    if text:find("^%[A%] .+ кикнул (%d+) игроков, которые стояли в АФК более 5 минут!") then
        local formaerror = text:match('^%[A%] .+ кикнул (%d+) игроков, которые стояли в АФК более 5 минут!')
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, 'Кикнуто игроков: '..formaerror)
    end
    if text:find("%[Ошибка%] %{......%}Введите: (.+)") then
        local formaerror = text:match('%[Ошибка%] %{......%}Введите: (.+)')
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, 'Используйте: '..formaerror)
    end
    if text:find("^У этого игрока уже есть бан чата!$") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "У этого игрока уже есть бан чата!")
    end
    if text:find("^Не нехочу!$") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Игрок уже находится в транспорте.")
    end
    if text:find("^Создать машину не удалось!$") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Создать машину не удалось!")
    end
    if text:find("^Номер Транспортного средства не может быть ниже 400 или выше") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Номер Транспортного средства не может быть ниже 400 или выше")
    end
    if text:find("^Не больше (%d+) символов!$") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Много символов!")
    end
    if text:find("^У этого игрока уже есть бан чата!$") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "У этого игрока уже есть бан чата!")
    end
    if text:find("Этот игрок уже в КПЗ!") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Этот игрок уже в КПЗ!")
    end
    if text:find("%[Подсказка%] {ffffff}/setmember (.+)") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Используйте: /setmember [id игрока] [id организации (1-31)] [ранг (1-9)]")
    end
    if text:find("Игрок не в д(.+)") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Игрок не в деморгане!")
    end
    if text:find("У игрока нет варн(.+)") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "У игрока нет варнов!")
    end
    if text:find("Игрок с ником (.+)") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Такого игрока нет в базе данных!")
    end
    if text:find("^Игрок не законектился еще!$") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Игрок не законектился еще!")
    end
    if text:find("^Игрок не в сети") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Игрок не в сети!")
    end
    if text:find("^Этот игрок уже в ДЕМОРГАНЕ!$") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Этот игрок уже в ДЕМОРГАНЕ!")
    end
    if text:find("^Извините , но такого человека сейчас нет в тюрьме!$") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Извините , но такого человека сейчас нет в тюрьме!")
    end
    if text:find("^Нельзя посадить меньше чем на 5 минут$") then
        bSettings.forms.isError[0] = true
        imgui.StrCopy(bSettings.forms.errortext, "Нельзя посадить меньше чем на 5 минут")
    end
end
end
...
local function delay36()
  if imgui.Combo(u8'Выбор темы', selected_theme, items, #theme_a) then
            ...
            imgui.Text(u8' - Цвет оформления скрипта')
            end
...
imgui.OnFrame(function() return WinState[0] end, function(player)
                    ...
                    if logregim[0] then
                        ...
                        if imgui.Button(u8"Изменить позицию чекера") then
                            bSettings.settings.logreg = true
                                checkCursor = true
                                sampSetCursorMode(4)
                                notify.game.push('ЛКМ для сохранения', '')
                                lua_thread.create(function()
                                while checkCursor do
                                    wait(0)
                        ...
                        notify.game.push('Двигайте пальцем', 'Зажмите пальцем и выбирете местоположение после чего отпустите палец что-бы сохранить позицию', -1)
                        WinState[0] = false
        ...
        imgui.Columns()
            ...
            if menu5 == 1 then
                ...
                imgui.SameLine()
                if imgui.Button(u8'Тема на blasthack', imgui.ImVec2(130, 30)) then
                    openLink('https://www.blast.hk')
                end
                imgui.SameLine()
                if imgui.Button(u8'ТГ канал', imgui.ImVec2(120, 30)) then
                ...
                imgui.Text(u8'Скрипт создан исключительно в админских целяхn\nтакже хочу сразу сказать, вкладка анти-бот(меньше половины), и PlayerList была создана с помощью пользователя ADust')
...
function checklvl(arg)
	...
	if status then
		...
		status = false
		pl = {}
		notify.game.push('Проверка остановлена', 'Проверка ботов остановлена')
		return
	end
	pl = {}
	if arg ~= "" and arg:find("^%d+$") then
		lua_thread.create(function ()
			...
			if #tablelvls == 0 then
                sampAddChatMessage('{4682B4}[Admin Helper] {FFFFFF}Аккаунты с указанным лвл не найдены. Попробуйте повторить попытку возможно не обновился TAB', -1)
            else
